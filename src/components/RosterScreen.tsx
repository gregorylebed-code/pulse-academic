      ...
      <main className="flex-1 px-4 py-5 max-w-6xl mx-auto w-full">
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {classes.map((cls: any) => {
            ...
            return (
              <div key={cls.id} className="bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col h-fit">
                <button
                  type="button"
                  onClick={() => setExpandedRosterClassId(isExpanded ? null : cls.id)}
                  className="w-full text-left focus:outline-none"
                >
                  <div className={`px-4 py-4 flex items-center justify-between transition-colors ${isExpanded ? 'bg-slate-50/50' : 'hover:bg-slate-50/30'}`}>
                    {isRenaming ? (
                      <div className="flex gap-2 flex-1" onClick={e => e.stopPropagation()}>
                        <input
                          ...
                          autoFocus
                          className="flex-1 text-sm bg-white rounded-xl px-3 py-1.5 outline-none border border-slate-200 focus:border-teal-300 font-semibold"
                        />
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-3 shrink-0 ml-2">
                          ...
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] font-bold text-slate-300 uppercase tracking-wider">{isExpanded ? 'Hide' : 'View'}</span>
                            <span className={`text-[10px] text-slate-300 transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`}>▼</span>
                          </div>
                        </div>
                      </>
                    )}
                ...
                {isExpanded && (
                  <div className="px-4 pb-5 pt-2 border-t border-slate-50 animate-in fade-in slide-in-from-top-1 duration-200">
